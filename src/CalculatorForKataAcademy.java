...
public class CalculatorForKataAcademy {
    public static void main(String[] args){
        ...
        String operation = "";  //Переменная хранящая информацию о том какая операция будет производиться
        ...
        System.out.println("Введите математический пример без пробелов: ");
        String s1 = s.nextLine();
        //Вычисления по знаку минус
        String[] operationMinus = s1.split("-"); // Разделяет строку по символу плюс
        for(String operands1: operationMinus) {
            if(operationMinus.length == 2){
                if(index == 0){
                    index++;
                    operation = "-";
                    ConvertToRomanNumerals convert = new ConvertToRomanNumerals();
                    convert.romanNumber = operands1;
                    check = convert.convertToRN();                                      //Делаем проверку на Арабские числа
                    if (check == 1000){
                        Checker checker = new Checker();
                        checker.numbercheck = operands1;
                        ...
                        proverka++;
                        firstOperand = Integer.parseInt(operands1);                     // Преобразует текстовый операнд в числовой операнд для дальнейших вычислений
                    }
                    else firstOperand = check;

                }
                else if (index == 1){
                    ConvertToRomanNumerals convert = new ConvertToRomanNumerals();      //Проверка на Арабские числа
                    convert.romanNumber = operands1;
                    check = convert.convertToRN();
                    if (check == 1000){
                        Checker checker = new Checker();
                        checker.numbercheck = operands1;
                        checker1 = checker.numcheck();
                        ...
                        proverka++;
                        secondOperand = Integer.parseInt(operands1);                    // Преобразует текстовый операнд в числовой операнд для дальнейших вычислений
                    }
        ...
        //Вычисления по знаку плюс
        String[] operationPlus = s1.split("\\+"); // Разделяет строку по символу плюс
        for(String operands1: operationPlus) {
            if(operationPlus.length == 2){
                if(index == 0){
                    index++;
                    operation = "+";
                    ConvertToRomanNumerals convert = new ConvertToRomanNumerals();
        ...
        //Вычисления по знаку деление
        String[] operationDivision = s1.split("/"); // Разделяет строку по символу деление
        for(String operands1: operationDivision) {
            if(operationDivision.length == 2){
                if(index == 0){
                    index++;
                    operation = "/";
                    ConvertToRomanNumerals convert = new ConvertToRomanNumerals();
        ...
        //Вычисления по знаку умножение
        String[] operationMultiplication  = s1.split("\\*"); // Разделяет строку по символу
        for(String operands1: operationMultiplication) {
            if(operationMultiplication.length == 2){
                if(index == 0){
                    index++;
                    operation = "*";
                    ConvertToRomanNumerals convert = new ConvertToRomanNumerals();
        ...
        if(proverka == 0){                                                              // Преобразуем результат в римские цифры
            ...
            else System.out.println(sumString);
        }
        else if (proverka == 1){
